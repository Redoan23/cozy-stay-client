...
const RoomAvailability = () => {
  ...
  const handleAvailability = (e) => {
    ...
    axios
      .get(`https://cozy-stay-server.vercel.app/rooms/check/${roomName}`)
      .then((res) => {
  ...
  return (
    <div>
      {error && (
        <SwalNotification roomData={error} date={date}></SwalNotification>
      )}
      {roomData && (
        <SwalNotification roomData={roomData} date={date}></SwalNotification>
      )}
      <div className="text-black pt-5">
        <form
          onSubmit={handleAvailability}
          className=" bg-white p-4 bg-opacity-40 rounded-md"
        >
          <div className="flex gap-3 lg:flex-row flex-col flex-wrap items-center justify-center">
            <select className=" p-3 rounded-md bg-yellow-50 border-none" name="rooms" id="rooms">
            ...
            <input
              className="p-3 rounded-md bg-yellow-50 border-none"
              name="date"
              id="date"
              type="date"
            />
            ...
            <input
              type="submit"
              value="Check"
              className=" text-base bg-yellow-500 border-none text-white text-center btn"
            />
          </div>
        </form>
      </div>
